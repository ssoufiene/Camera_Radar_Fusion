...
def evaluate_model(model, eval_loader, device, class_names, iou_threshold=0.5, num_viz=5):
    """
    Evaluate model, plot PR curve, and visualize predictions vs ground truth.
    num_viz: number of samples to visualize
    """
    model.eval()
    preds_by_class, gts_by_class = defaultdict(list), defaultdict(list)

    # Store images and data for visualization
    all_data = []
    ...
    # Collect all detections into one list
...
def visualize_predictions(all_data, class_names, num_viz=5):
    """Visualize predicted boxes vs ground truth without class labels."""
    ...
    for idx in range(num_samples):
        ...
        ax_gt.set_title(f'Ground Truth (Sample {idx + 1}) - {num_gt} boxes', fontsize=12, fontweight='bold')
        ...
        ax_pred.axis('off')
        num_pred = len(data['pred_boxes'])
        ax_pred.set_title(f'Predictions (Sample {idx + 1}) - {num_pred} boxes', fontsize=12, fontweight='bold')
